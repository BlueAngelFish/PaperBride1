...
public class PhotoPuzzle : MonoBehaviour
{
    ...
    private static List<PhotoPuzzle> allArticles = new List<PhotoPuzzle>(); //check for overlap and flip 
    ...
    void Start()
    {
        ...
        {
            totalArticles = FindObjectsOfType<PhotoPuzzle>().Length;
        }
    ...
    bool IsPositionOverlapping(Vector3 position) //check if pieces are overlapping 
    {
        ...
        foreach (PhotoPuzzle other in allArticles)
        {
            if (other == this) continue;
